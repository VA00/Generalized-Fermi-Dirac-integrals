  ...
  TimeConstrained[If[\[Eta] > 4,
    Check[
      NIntegrate[Evaluate@integrand, {x, 0, \[Eta]}, 
       ...
       MaxRecursion -> 64, WorkingPrecision -> 1024, PrecisionGoal->64], Infinity] +
     Check[
      NIntegrate[Evaluate@integrand, {x, \[Eta], Infinity}, 
       Method -> "GlobalAdaptive", MinRecursion -> 11, 
       MaxRecursion -> 29, WorkingPrecision -> 1024, PrecisionGoal->64], Infinity],
    Check[
...
(* \[Theta]TBL = {1}; *)
\[Eta]TBL = 
  Join[Table[-2^i, {i, 1024, -1, -delta}], {0}, 
   Table[2^i, {i, -1, 26, delta}]];
...
refTBL=Select[refTBLtmp, NumericQ[Total[#]] && Precision[Total[#]]>32 &];
...
Print[DateString[]];
Print[inputTBLall//Length]
Print[Length[inputTBL]]
Print[Length[refTBL]]
